...
// ─── STORE ────────────────────────────────────────────────────────────────────
const useStore = () => {
  const [users,       setUsers]       = useState(() => LS.get("cc_users", []));
  const [incidents,   setIncidents]   = useState(() => LS.get("cc_incidents", []));
  const [chats,       setChats]       = useState(() => LS.get("cc_chats", {}));
  const [currentUser, setCurrentUser] = useState(() => LS.get("cc_currentUser", null));
  const [view,        setView]        = useState(() => {
    const u = LS.get("cc_currentUser", null);
    if (!u) return "auth";
    return u.role === "admin" ? "admin" : "staff";
  });

  // Persist on every change
  useEffect(() => { LS.set("cc_users",       users);       }, [users]);
  useEffect(() => { LS.set("cc_incidents",   incidents);   }, [incidents]);
  useEffect(() => { LS.set("cc_chats",       chats);       }, [chats]);
  useEffect(() => { LS.set("cc_currentUser", currentUser); }, [currentUser]);

  // Keep currentUser fresh whenever users array updates
  useEffect(() => {
    if (!currentUser) return;
    const fresh = users.find(u => u.id === currentUser.id);
    if (fresh && JSON.stringify(fresh) !== JSON.stringify(currentUser)) {
      setCurrentUser(fresh);
      LS.set("cc_currentUser", fresh);
    }
  }, [users]);

  /* ── Auth ── */
  const registerUser = (data) => {
    // Always read latest from LS to avoid stale closure
    const latest = LS.get("cc_users", []);
    if (latest.find(u => u.username === data.username)) return { error: "Username already taken." };
    const nu = {
      id: Date.now().toString(),
      name: data.name, username: data.username, password: data.password,
      role: data.role, department: data.department || null,
      status: "available", joinedAt: new Date().toISOString(),
    };
    const next = [...latest, nu];
    setUsers(next); LS.set("cc_users", next);
    setCurrentUser(nu); LS.set("cc_currentUser", nu);
    setView(nu.role === "admin" ? "admin" : "staff");
    return { user: nu };
  };

  const loginUser = (username, password) => {
    // Always read from LS so a refresh doesn't lose registrations
    const latest = LS.get("cc_users", []);
    const found = latest.find(u => u.username === username && u.password === password);
    if (!found) return { error: "Invalid username or password." };
    setUsers(latest); // sync state
    setCurrentUser(found); LS.set("cc_currentUser", found);
    setView(found.role === "admin" ? "admin" : "staff");
    return { user: found };
  };

  const logout = () => { setCurrentUser(null); LS.set("cc_currentUser", null); setView("auth"); };

  const refreshFromStorage = () => {
    const latestUsers = LS.get("cc_users", []);
    const latestIncidents = LS.get("cc_incidents", []);
    const latestChats = LS.get("cc_chats", {});
  ...
  const updateStatus = (userId, status) => {
    setUsers(prev => {
      ...
      LS.set("cc_users", next); return next;
    });
    setCurrentUser(prev => {
      if (!prev || prev.id !== userId) return prev;
      const next = { ...prev, status }; LS.set("cc_currentUser", next); return next;
    });
  };
  ...
  const fireAlert = (data) => {
    ...
    setIncidents(prev => { const next = [inc, ...prev]; LS.set("cc_incidents", next); return next; });
    const msg = {
      id: "sys_" + Date.now(), sender: "System", role: "system",
      text: `🚨 Alert fired — ${data.department} needed at Room ${data.room}${data.floor ? `, Floor ${data.floor}` : ""}. Severity: ${data.severity}.${data.note ? " Note: " + data.note : ""}`,
      time: new Date().toLocaleTimeString(),
    };
    setChats(prev => { const next = { ...prev, [inc.id]: [msg] }; LS.set("cc_chats", next); return next; });
    return inc;
  };

  const acceptIncident = (incidentId, user) => {
    setIncidents(prev => {
      const next = prev.map(i => i.id === incidentId
        ? { ...i, status: "active", acceptedBy: user, acceptedAt: new Date().toISOString() } : i);
      LS.set("cc_incidents", next); return next;
    });
    updateStatus(user.id, "busy");
    addSysMsg(incidentId, `✅ ${user.name} accepted — en route.`);
  };

  const resolveIncident = (incidentId) => {
    setIncidents(prev => {
      const inc = prev.find(i => i.id === incidentId);
      const next = prev.map(i => i.id === incidentId ? { ...i, status: "resolved", resolvedAt: new Date().toISOString() } : i);
      LS.set("cc_incidents", next);
      if (inc?.acceptedBy) updateStatus(inc.acceptedBy.id, "available");
      return next;
    });
    addSysMsg(incidentId, "🏁 Incident resolved.");
  };

  const addSysMsg = (incidentId, text) => {
    setChats(prev => {
      const next = { ...prev, [incidentId]: [...(prev[incidentId] || []), {
        id: "sys_" + Date.now(), sender: "System", role: "system",
        text, time: new Date().toLocaleTimeString(),
      }]};
      LS.set("cc_chats", next); return next;
    });
  };

  const sendMessage = (incidentId, text, file) => {
    if (!text.trim() && !file) return;
    const msg = {
      id: Date.now().toString(),
      sender: currentUser.name, role: currentUser.role, userId: currentUser.id,
      text: text.trim(), file: file || null, time: new Date().toLocaleTimeString(),
    };
    setChats(prev => {
      const next = { ...prev, [incidentId]: [...(prev[incidentId] || []), msg] };
      LS.set("cc_chats", next); return next;
    });
  };

  /* ── Alert routing ──
     Nurses see ONLY "Nurse" department alerts.
     Doctors see ALL non-nurse open alerts (admin doesn't know which specific
     doctor is free — that's why we broadcast to all available doctors).
  */
  const incidentsForUser = (user) => {
    if (!user || user.role === "admin") return incidents;
    return incidents.filter(i => {
      if (i.status !== "open") return false;
      if (user.role === "nurse") return i.department === "Nurse";
      return i.department !== "Nurse"; // all doctors see all non-nurse alerts
    });
  };
        ...
        <div style={{ padding:"10px 18px", borderTop:"1px solid var(--border)", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
            <textarea value={text} onChange={e => setText(e.target.value)}
              ...
              style={{ flex:1, resize:"none", borderRadius:10, fontSize:13, padding:"10px 14px" }}
    ...
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:20 }}>
          ...
          {tab==="staff" && <>
            <h2 style={{ fontFamily:"var(--font-head)", fontSize:18, fontWeight:700, marginBottom:16 }}>All staff</h2>
            {staff.length===0 && (
              <div style={{ textAlign:"center", padding:60, color:"var(--muted)" }}>
                <div style={{ fontSize:48, marginBottom:12 }}> </div>
